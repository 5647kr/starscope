...
export default function QnaPost() {
  const { readPost, updatePost, commu_qna } = usePostStore();
  ...
  const handleChange = async (e) => {
    ...
    await updatePost(id, { recall: newValue }, "commu_qna");
  };

  const filteredQna = commu_qna
    .filter((item) => {
  ...
  const hasAnswered = commu_qna.some((item) => item.recall === true);
  const hasUnanswered = commu_qna.some((item) => item.recall === false);

  let message = "";
  if (sortTable === "true" && !hasAnswered)
    message = "답변완료한 문의가 없습니다.";
  if (sortTable === "false" && !hasUnanswered)
    message = "모든 문의의 답변을 완료하였습니다.";
      ...
      <TableWrap>
        {commu_qna.length > 0 ? (
          filteredQna.length > 0 ? (
            ...
            <EmptyBox>
              <MailQuestionMark />
              <strong>{message || "해당 조건의 문의가 없습니다."}</strong>
            </EmptyBox>
          )
        ) : (
          <EmptyBox>
